...
export class InventoryService {
  ...
  addFieldToInventory(inventoryId: string, fieldData: CreateFieldRequest): Observable<InventoryFieldDto> {
    const payload = {
      ...fieldData,
      type: Number(fieldData.type)
    };
    
    return this.http.post<InventoryFieldDto>(
      `${this.apiUrl}/${inventoryId}/fields`,
      payload
    );
  }
  ...
  saveInventorySettings(
    id: string,
    payload: Partial<InventoryDetailsDto>,
    version?: string
  ): Observable<{ result: InventoryDetailsDto; version?: string }> {
    const headers = version
      ? new HttpHeaders({ 'If-Match': version })
      : undefined;
    return this.http
      .put<InventoryDetailsDto>(`${this.apiUrl}/${id}/settings`, payload, {
        headers,
      })
      .pipe(map((res) => ({ result: res, version: (res as any).version })));
  }
  ...
  addItemToInventory(inventoryId: string, itemData: CreateItemRequest): Observable<ItemDetailsDto> {
    return this.http.post<ItemDetailsDto>(
      `${this.apiUrl}/${inventoryId}/items`,
      itemData
    );
  }
  updateItem(
    inventoryId: string,
    itemId: string,
    item: Partial<ItemDetailsDto>
  ): Observable<ItemDetailsDto> {
    return this.http.put<ItemDetailsDto>(
  ...
  searchTags(prefix: string): Observable<string[]> {
    ...
    return this.http.get<string[]>(
      `${this.apiUrl}/tags/autocomplete?q=${encodeURIComponent(prefix)}`
    );
  }
  ...
  searchUsers(q: string): Observable<UserAccessDto[]> {
    if (!q) return of([]);
    return this.http.get<UserAccessDto[]>(
  ...
  postDiscussionMessage(inventoryId: string, text: string): Observable<any> {
    return this.http.post(`/api/inventories/${inventoryId}/discussion`, {
      text,
    });
  }
